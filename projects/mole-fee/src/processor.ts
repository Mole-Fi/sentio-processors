  ...
  .onTimeInterval(async (dynamicFieldObjects, ctx) => {
    try {
      ...
      for (const field of fields) {
        ...
        ctx.meter.Gauge("savings_borrowing_interest").record(savings_borrowing_interest, { coin_symbol, project: "mole-fee" })
        ...
        ctx.meter.Gauge("savings_lending_interest").record(savings_lending_interest_apy, { coin_symbol, project: "mole-fee" })
  ...
  .onTimeInterval(async (dynamicFieldObjects, ctx) => {
    try {
      ...
      for (const field of fields) {
        ...
        //@ts-ignore
        ctx.meter.Gauge("savings_fee").record(savingsFee, { coin_symbol, project: "mole-fee" })

        //@ts-ignore
        ctx.meter.Gauge("savings_fee_usd").record(savingsFeeUsd, { coin_symbol, project: "mole-fee" })
  ...
  .onEventDepositEvent(
    async (event, ctx) => {
      ...
      ctx.meter.Counter("supplied_amount").add(amount_usd, { pool_address: pool, underlying_token_address: coinType, underlying_token_symbol: coin_symbol, 
        supplier_address: event.sender, collateral_amount: "NA", collateral_amount_usd: "NA", project: "mole-fee" })
  ...
  .onTimeInterval(async (self, _, ctx) => {
    ...
    try {
      ...
      for (let [key, value] of sharesMap) {
        ...
        ctx.meter.Gauge("lyf_usd_farm_user_usd").record(lyf_usd_farm_user_usd, {farmPairName, positionId, project: "mole-fee" })
